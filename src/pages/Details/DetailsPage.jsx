...
const DetailsPage = () => {
	...
	const { data: videos, loading: videosLoading } = useFetchData(`/${mediaType}/${id}/videos`);
	...
	return (
		<div className={`w-[100%] h-[100vh]`}>
			...
			<div className="content relative w-[1200px] mx-auto mt-5">
				<div className="Hero flex items-center gap-10">
					<div className="left">
						<div className="hero-img w-[330px]">
							<LazyLoadingImage
								imgUrl={
					...
					<div className="right mt-3">
						<div className="title text-white mb-[10px]">
							<h4 className="text-4xl font-bold">{data.title}</h4>
							<span className="text-xl italic opacity-[0.6]">
								{data.tagline}
							</span>
						</div>
							...
							<ul className="flex list-none items-center gap-4">
								{data.genres?.map((genre) => (
									<li
										...
										key={genre.id}
									>
										<h4 className="text-black font-semibold">{genre.name}</h4>
									</li>
								))}
							</ul>
						</div>
						<div className="rating_trailer my-[25px]">
							<div className="flex items-center gap-10">
								<div className="playIcon w-[8%]">
									<CircularProgressbar
								...
								<div className="playBtn flex items-center gap-[10px] cursor-pointer">
									<span className="text-4xl text-white opacity-70 ">
										<FaPlay />
									</span>
									<h4 className="text-white opacity-90 text-2xl">
										Watch Trailer
									</h4>
								</div>
							</div>
						</div>
						<div className="overview my-[25px] text-white">
                ...
                {!videosLoading ? <VideosList data={videos}/> : ""}
			</div>
		</div>
	);
};
